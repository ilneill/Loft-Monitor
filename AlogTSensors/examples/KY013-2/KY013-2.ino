/*
Simple KY013 analog temperature sensor example sketch 2.
...
Assumed KY013 temperature sensor design:
 - The KY013 resistance that varies is R1.
 ...
 (5V)     |
         ---
         |R| R1 = KY013 Thermistor
         ---
          |----- vOut --> Arduino Pin A2 (KY013PIN)
         ---
         |R| R2 = 100K Resistor (external only, the onboard resistor is bypassed)
         ---
          |
 Gnd ------
*/
...
KY013 myKY013(KY013PIN, R2, false);         //The balance resistor is R2 (not R1).
...
void loop() {
  ...
  unsigned int avgADC = myKY013.readADC();                    //Get a new reading, 32 samples, 2ms apart, and then averaged.
  ...
  float myKY013R1i = myKY013.calcR1(avgADC);                  //Pass the KY013 ADC reading already taken, else this function will get a new reading.
  //Calculate the KY013 resistance using the calculated voltage.
  float myKY013R1f = myKY013.calcR1(vOut);                    //Pass the KY013 voltage already calculated, else this function will get a new reading.
  float temperatureCi = myKY013.readTemperatureC(myKY013R1i); //Pass the KY013 i resistance already calculated, else this function will get a new reading.
  float temperatureCf = myKY013.readTemperatureC(myKY013R1f); //Pass the KY013 f resistance already calculated, else this function will get a new reading.
  ...
  Serial.print(myKY013R1i);
  Serial.println("R");
  Serial.print("  Resistance f  = ");
  Serial.print(myKY013R1f);
  Serial.println("R");
